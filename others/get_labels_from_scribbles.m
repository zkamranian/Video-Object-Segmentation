...
switch labelParam.fea_style
   ...
   case 'features'
        ...
        %%%% GMM
        nfeat = size(scribbled_sp_fea_all,1);
        n_labels = 6;
        colorsSP = feature;
        scribbled_sp_colors_all = scribbled_sp_fea_all;
        GMMparam = labelParam;
        ...
        for i=1:ncentersB
            L = chol(inv(sigmaB(:,:,i) + stabB*eye(nfeat)));
            tmp = L*(A - repmat(muB(:,i),1,n));
        %     mean( sum( (A - repmat(muB(:,i),1,n)).^2, 1 ))
            wo(:,i) = -log(unarypotb(i)) + 0.5*log(det(sigmaB(:,:,i) + stabB*eye(nfeat))) + 0.5* sum( tmp.^2, 1)'; 
        end
        
        for i=1:ncentersF
            L = chol(inv(sigmaF(:,:,i)+stabF*eye(nfeat)));
            tmp = L*(A - repmat(muF(:,i),1,n));
            wb(:,i) = -log(unarypotF(i)) + 0.5*log(det(sigmaF(:,:,i) + stabF*eye(nfeat))) + 0.5* sum( tmp.^2, 1)'; 
        end
