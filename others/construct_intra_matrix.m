function [ M_intra ,Y0] = construct_intra_matrix( color_SP, labelInd, edges_s1,options )
%CONSTRUCT_INTRA_MATRIX Summary of this function goes here
%   Detailed explanation goes here
%  color_SP=colors_s{10}'; labelInd=label_ind;  edges_s1=edges_s{10};
if nargin <  4
...
nb(1,:) = [1:N];
connect_nb = cell(1,N);%the neighbours for each superpixels  ÿ��point�ڽӵĽڵ�����    
temp1 = edges_s1(:,1);
temp2 = edges_s1(:,2);   
for i = 1:N
    temp =  temp1(temp2==i);
    ...
    [~,index] = sort(dist);
    temp =[i; temp(index)];% the nearest sort according the distance, including itself ��ݾ��룬�õ��������� ���Լ�
    if length(temp)>k
